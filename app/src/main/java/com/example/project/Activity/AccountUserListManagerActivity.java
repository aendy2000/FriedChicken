...
public class AccountUserListManagerActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_account_user_list_manager);
        matching();
        load();
    }
        ...
        reference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (DataSnapshot data : snapshot.getChildren()){
                    if(data.getValue().toString().substring(data.getValue().toString().indexOf("Role") + 5, data.getValue().toString().indexOf(", SDT")).equals("User")){
                        String ten = data.getValue().toString().substring(data.getValue().toString().indexOf("Ten") + 4, data.getValue().toString().indexOf(", Password"));
                        String hinhanh = data.getValue().toString().substring(data.getValue().toString().indexOf("HinhDaiDien") + 12, data.getValue().toString().indexOf(", GioHang"));
                ...
                adapter = new AccountListUserAdapter(AccountUserListManagerActivity.this, list);
    ...
    private void matching() {
        cancel = (ImageView) findViewById(R.id.img_account_user_list_cancel);
        recyclerView = (RecyclerView) findViewById(R.id.rcv_list_account_user);
